...
PROCEDURE ensureAttractionFactors(CONST timeStepIndex:longint);
  ...
  FUNCTION straightAttraction(CONST rx,ry:TmyFloat):T_2dVector;
    ...
    begin
      ...
      if f<20 then begin
	    if f<f0-q then f:=-1/f else
		if f<f0+q then f:=(f-f0)/(q*f)
		          else f:= 1/f;
	    result[0]:=rx*f;
        result[1]:=ry*f;
  ...
  begin
    if not(attractionInitialized) or (timeStepIndex mod 10=0) then begin
	  q:=5*(1+cos(timeStepIndex*20*pi/5000));
	  f0:=abs(20-40*timeStepIndex/5000);
      append(logHandle);
	  writeln(logHandle,'Initializing gravity factors; q=',q:0:3,'; f0=',f0:0:3);
	  close(logHandle);
